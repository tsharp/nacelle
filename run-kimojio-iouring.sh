...
pkill -f 'kimojio-iouring-server' 2>/dev/null || true
sleep 0.2

# Require pre-built artifacts; run ./build-all.sh first
if [[ ! -x artifacts/kimojio-iouring-server || ! -x artifacts/nacelle-stress-test ]]; then
    echo "artifacts not found — run ./build-all.sh first"
    exit 1
fi

# Start server in background
./artifacts/kimojio-iouring-server \
    --bind "$BIND" \
...
echo "--- kimojio io_uring  threads=$SERVER_THREADS  connections=$CONNECTIONS  pipeline=$PIPELINE ---"
