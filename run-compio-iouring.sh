...
pkill -f 'compio-iouring-server' 2>/dev/null || true
sleep 0.2

# Require pre-built artifacts; run ./build-all.sh first
if [[ ! -x artifacts/compio-iouring-server || ! -x artifacts/nacelle-stress-test ]]; then
    echo "artifacts not found — run ./build-all.sh first"
    exit 1
fi

# Start server in background
./artifacts/compio-iouring-server \
    --bind "$BIND" \
...
echo "--- compio io_uring  threads=$SERVER_THREADS  connections=$CONNECTIONS  pipeline=$PIPELINE ---"
