// Compio (io_uring / IOCP / polling thread-per-core) stress server.
//
// Each OS thread runs its own compio runtime and accepts connections on the
...
//       --no-default-features --features compio-runtime
//   ./target/release/stress-compio [options]
...
fn main() {
    let config = parse_args(std::env::args().skip(1), "compio").expect("invalid args");
    let server = build_server(&config).expect("failed to build server");
    let addr = config.bind;
    let n_threads = config.server_threads.max(1);

    println!("nacelle-stress-server (compio) listening on {addr} (threads={n_threads})");

    let threads: Vec<_> = (0..n_threads)
        .map(|i| {
            let server = server.clone();
            std::thread::Builder::new()
                .name(format!("compio-{i}"))
                .spawn(move || {
