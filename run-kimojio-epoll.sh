...
pkill -f 'kimojio-epoll-server' 2>/dev/null || true
sleep 0.2

# Require pre-built artifacts; run ./build-all.sh first
if [[ ! -x artifacts/kimojio-epoll-server || ! -x artifacts/nacelle-stress-test ]]; then
    echo "artifacts not found — run ./build-all.sh first"
    exit 1
fi

# Start server in background
./artifacts/kimojio-epoll-server \
    --bind "$BIND" \
...
echo "--- kimojio epoll  threads=$SERVER_THREADS  connections=$CONNECTIONS  pipeline=$PIPELINE ---"
