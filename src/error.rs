...
impl Display for CascadeError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingHandler => f.write_str("at least one handler or a default handler is required"),
            Self::DuplicateHandler(opcode) => write!(f, "duplicate handler registration for opcode {opcode}"),
            Self::UnknownOpcode(opcode) => write!(f, "unknown opcode {opcode}"),
            Self::InvalidFrame(message) => write!(f, "invalid frame: {message}"),
            Self::FrameTooLarge { len, max } => write!(f, "frame length {len} exceeds configured maximum {max}"),
...
impl From<tokio::task::JoinError> for CascadeError {
